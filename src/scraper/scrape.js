...
(async function() {
  ...
  console.log(` ⚾️ Fetching all zenhub issues in release '${release.title}'`);
  // array of issues with issue_number and repo_id
  const issues_by_release = await zenhub.getIssuesByRelease(release.release_id);
  // group issues by repo
  const repos = _.groupBy(issues_by_release, "repo_id");

  console.log(" 🏈 Fetching the corresponding github issues by repository...");
